...
def normalQueryWithPreidcateSpecified(predicate):
  sparql = SPARQLWrapper('http://localhost:7200/repositories/altair')
  sparql.setQuery("""PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
                        PREFIX owl: <http://www.w3.org/2002/07/owl#>
                        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
  ...
  for result in results["results"]["bindings"]:
    list_of_object.append({'subject': (result["s"]["value"]), 'predicate': ':'+predicate, 'object': (result["o"]["value"])})
    f.write('subject:' + result["s"]["value"] + ' predicate:'+':to' + ' object:' + result["o"]["value"])
    f.write('\n')
...
def normalQueryAllSelectedStatmentSpecified(selectStatmentRobaInternaAllaParentesiGraffa):
  sparql = SPARQLWrapper('http://localhost:7200/repositories/altair')
  sparql.setQuery("""PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
                        PREFIX owl: <http://www.w3.org/2002/07/owl#>
                        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
                        ...
                        PREFIX r: <http://www.disit.org/altair/resource/>
                        select *  {
                            """+selectStatmentRobaInternaAllaParentesiGraffa+"""
                        }""")
  sparql.setReturnFormat(JSON)
  results = sparql.query().convert()
  list_of_object = []
  ...
  for result in results["results"]["bindings"]:
    list_of_object.append({'subject': (result["s"]["value"]), 'predicate': (result["p"]["value"]), 'object': (result["o"]["value"])})
...
##?s ?p ?o.
##?p rdfs:subPropertyOf* :to. # il property path va messo se è sotto di almeno due livelli
##FILTER (?p != :to).
